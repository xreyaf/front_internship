...
class Calculator {
  ...
  compute() {
    ...
    const current = this.currenOperand;
...
window.onload = () => {
  ...
  getLct.addEventListener('click', () => {
    navigator.geolocation.getCurrentPosition(
      (res) => {
        console.log(
          `Текущее местоположение: ${res.coords.latitude}, ${res.coords.longitude}`
        );
...
function fetchedData(data) {
  return data
    .map(
      (data) =>
        `Имя репозитория: ${data.name}, создан: ${new Date(
          data.created_at
        ).toLocaleDateString()}, последнее обновление: ${new Date(
          data.updated_at
        ).toLocaleDateString()}. Описание: ${data.description}`
    )
    .join('\n');
}
